...
export default function LinenProfile() {
  ...
  if (isLoading) return <div className="space-y-4"><Skeleton className="h-64 rounded-xl" /><Skeleton className="h-96 rounded-xl" /></div>
  ...
  return (
    <div className="space-y-5">
      <Breadcrumb items={[
        ...
        { label: 'Inventory', href: '/linen' },
        { label: linen.linen_id },
      ]} />

      <div className="flex flex-col lg:flex-row gap-5">
        {/* Left column — QR, Barcode, Info */}
        <div className="lg:w-80 space-y-4">
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-6 flex flex-col items-center gap-4">
              ...
              <div className="w-full text-center">
                ...
                <p className="mt-2 text-lg font-bold font-mono text-[var(--text-primary)]">{linen.linen_id}</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-4">
              <p className="text-xs font-semibold text-[var(--text-muted)] uppercase mb-2">Current Status</p>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: stCfg.color }} />
                <span className="text-lg font-bold" style={{ color: stCfg.color }}>{stCfg.label}</span>
              </div>
              {linen.location && <p className="text-xs text-[var(--text-muted)] mt-1">Location: {linen.location}</p>}
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-4">
              <p className="text-xs font-semibold text-[var(--text-muted)] uppercase mb-3">Quick Actions</p>
        ...
        <div className="flex-1 space-y-4">
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-5">
              <h2 className="text-lg font-bold text-[var(--text-primary)] mb-4" style={{ fontFamily: '"Spectral", Georgia, serif' }}>Item Details</h2>
              ...
              {linen.notes && (
                <div className="mt-4 p-3 bg-[var(--surface)] rounded-lg">
                  <p className="text-xs text-[var(--text-muted)] font-medium mb-1">Notes</p>
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-5">
              <h2 className="text-lg font-bold text-[var(--text-primary)] mb-4" style={{ fontFamily: '"Spectral", Georgia, serif' }}>Movement History</h2>
              {events?.items?.length ? (
                <div className="space-y-0">
                  {events.items.map((evt, i) => {
                    ...
                    return (
                      <div key={evt.id} className="flex gap-3 relative pb-4">
                        {i < events.items.length - 1 && <div className="absolute left-[7px] top-5 bottom-0 w-px bg-[var(--border)]" />}
                        <div className="w-[15px] h-[15px] rounded-full border-2 flex-shrink-0 mt-0.5" style={{ borderColor: toCfg.color, backgroundColor: i === 0 ? toCfg.color : 'transparent' }} />
