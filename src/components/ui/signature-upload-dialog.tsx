...
export function SignatureUploadDialog({ open, slots, onClose, onConfirm }: SignatureUploadDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={o => { if (!o) onClose() }}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add signature to print</DialogTitle>
          <DialogDescription>
            Upload a PNG signature for this print. It is used for this print only and is never saved to
            the database.
          </DialogDescription>
        </DialogHeader>
        <DialogBody>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onConfirm(payload)} className="w-full cursor-pointer gap-2">
            <Printer size={15} /> Print with signature
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
